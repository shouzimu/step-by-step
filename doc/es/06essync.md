...
public class MemberCanalClient {
    ...
    public void initElasticSearch() throws UnknownHostException {
        Settings settings = Settings.builder().put("cluster.name", "my-application-1").put("xpack.security.user", "用户名:密码").put("client.transport.sniff", false).build();
        transportClient = new PreBuiltXPackTransportClient(settings);
        TransportAddress transportAddress = new InetSocketTransportAddress(InetAddress.getByName("ip"), "端口");
