...
import com.dh.antlr.sqlbuff.misc.ParentSiblingListKey;
import com.dh.antlr.sqlbuff.misc.RuleAltKey;
import com.dh.antlr.sqlbuff.misc.SiblingListStats;
import com.dh.antlr.sqlbuff.validation.FeatureVectorAsObject;
import com.dh.antlr.sqlbuff.walkers.CollectSiblingLists;
import com.dh.antlr.sqlbuff.walkers.CollectTokenPairs;
...
import static com.dh.antlr.sqlbuff.Tool.*;
...
public class Corpus {
	...
	List<com.dh.antlr.sqlbuff.InputDocument> documents; // A list of all input docs to train on

	public List<com.dh.antlr.sqlbuff.InputDocument> documentsPerExemplar; // an entry for each featureVector
	public List<int[]> featureVectors;
	public List<Integer> injectWhitespace;
	public List<Integer> hpos;

	public void addExemplar(com.dh.antlr.sqlbuff.InputDocument doc, int[] features, int ws, int hpos) {
	...
	public Corpus(String rootDir, LangDescriptor language) throws Exception {
		...
		if ( documents==null ) {
			...
			documents = com.dh.antlr.sqlbuff.Tool.load(allFiles, language);
		}
	}

	public Corpus(List<com.dh.antlr.sqlbuff.InputDocument> documents, LangDescriptor language) {
	...
	public void collectTokenPairsAndSplitListInfo() throws NoSuchMethodException, InstantiationException, IllegalAccessException, java.lang.reflect.InvocationTargetException {
		...
		for (com.dh.antlr.sqlbuff.InputDocument doc : documents) {
	...
	public void trainOnSampleDocs() throws Exception {
		...
		for (com.dh.antlr.sqlbuff.InputDocument doc : documents) {
			...
			com.dh.antlr.sqlbuff.Trainer trainer = new com.dh.antlr.sqlbuff.Trainer(this, doc, language.indentSize);
	...
	public void randomShuffleInPlace() {
		...
		for (int i=n-1; i>=1; i--) {
			...
			com.dh.antlr.sqlbuff.InputDocument tmpD = documentsPerExemplar.get(i);
	...
	public void buildTokenContextIndex() {
		...
		for (int i = 0; i<featureVectors.size(); i++) {
			...
			int curTokenRuleIndex = features[com.dh.antlr.sqlbuff.Trainer.INDEX_PREV_EARLIEST_RIGHT_ANCESTOR];
			int prevTokenRuleIndex = features[com.dh.antlr.sqlbuff.Trainer.INDEX_EARLIEST_LEFT_ANCESTOR];
			...
			wsFeaturesToExemplarIndexes.map(new FeatureVectorAsObject(features,   com.dh.antlr.sqlbuff.Trainer.FEATURES_INJECT_WS), i);
			hposFeaturesToExemplarIndexes.map(new FeatureVectorAsObject(features, com.dh.antlr.sqlbuff.Trainer.FEATURES_HPOS), i);
		}
	}
}
