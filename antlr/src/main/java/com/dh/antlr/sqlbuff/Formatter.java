...
import com.dh.antlr.sqlbuff.validation.TokenPositionAnalysis;
import com.dh.antlr.sqlbuff.walkers.IdentifyOversizeLists;

import java.util.List;
import java.util.Map;
import java.util.Vector;

import static com.dh.antlr.sqlbuff.Dbg.normalizedLevenshteinDistance;
import static com.dh.antlr.sqlbuff.Dbg.tokenText;
import static com.dh.antlr.sqlbuff.Tool.tokenize;
import static com.dh.antlr.sqlbuff.Trainer.*;
import static com.dh.antlr.sqlbuff.misc.BuffUtils.filter;

public class Formatter {
	public static final int DEFAULT_K = 11;

	public com.dh.antlr.sqlbuff.Corpus corpus;
	...
	public int k;
	public com.dh.antlr.sqlbuff.FeatureMetaData[] wsFeatures = FEATURES_INJECT_WS;
	public com.dh.antlr.sqlbuff.FeatureMetaData[] hposFeatures = FEATURES_HPOS;

	public com.dh.antlr.sqlbuff.InputDocument originalDoc; // used only for debugging
	public com.dh.antlr.sqlbuff.InputDocument testDoc;
	...
	public Formatter(com.dh.antlr.sqlbuff.Corpus corpus, int indentSize, int k,
                     com.dh.antlr.sqlbuff.FeatureMetaData[] wsFeatures, com.dh.antlr.sqlbuff.FeatureMetaData[] hposFeatures)
	{
	...
	public Formatter(com.dh.antlr.sqlbuff.Corpus corpus, int indentSize) {
		this.corpus = corpus;
//		k = (int)Math.sqrt(corpus.X.size());
	...
	public String format(com.dh.antlr.sqlbuff.InputDocument doc, boolean collectAnalysis) throws Exception {
		...
		this.testDoc = doc;//com.dh.antlr.sqlbuff.InputDocument.dup(doc); // make copy of doc,
		// getting new tokens, tree
		...
		line = com.dh.antlr.sqlbuff.Tool.count(prefix, '\n') + 1;
		...
		for (int i = com.dh.antlr.sqlbuff.Trainer.ANALYSIS_START_TOKEN_INDEX; i<realTokens.size(); i++) { // can't process first token
	...
	public void processToken(int indexIntoRealTokens, int tokenIndexInStream, boolean collectAnalysis) {
		...
		int injectNL_WS = wsClassifier.classify(k, features, com.dh.antlr.sqlbuff.Trainer.MAX_WS_CONTEXT_DIFF_THRESHOLD);
		...
		if ( (injectNL_WS&0xFF)==CAT_INJECT_NL ) {
			newlines = com.dh.antlr.sqlbuff.Trainer.unnlcat(injectNL_WS);
		}
		else if ( (injectNL_WS&0xFF)==CAT_INJECT_WS ) {
			ws = com.dh.antlr.sqlbuff.Trainer.unwscat(injectNL_WS);
		}
		...
		if ( newlines>0 ) {
			output.append(com.dh.antlr.sqlbuff.Tool.newlines(newlines));
			line+=newlines;
			charPosInLine = 0;
			...
			if ( (alignOrIndent&0xFF)==CAT_ALIGN_WITH_ANCESTOR_CHILD ) {
				...
				if ( tokensOnPreviousLine.size()>0 ) {
					...
					output.append(com.dh.antlr.sqlbuff.Tool.spaces(indentCol));
				}
			}
			...
			output.append(com.dh.antlr.sqlbuff.Tool.spaces(ws));
			charPosInLine += ws;
		}
	...
	public void indent(int indentCat, TerminalNode node) {
		...
		if ( indentCat==CAT_INDENT ) {
			if ( firstTokenOnPrevLine!=null ) { // if not on first line, we cannot indent
				...
				output.append(com.dh.antlr.sqlbuff.Tool.spaces(indentedCol));
			}
			else {
				// no prev token? ok, just indent from left edge
				charPosInLine = indentSize;
				output.append(com.dh.antlr.sqlbuff.Tool.spaces(indentSize));
			}
			return;
		}
		int[] deltaChild = com.dh.antlr.sqlbuff.Trainer.unindentcat(indentCat);
		...
		ParserRuleContext ancestor = com.dh.antlr.sqlbuff.Trainer.getAncestor(earliestLeftAncestor, deltaFromAncestor);
		Token start = null;
		if ( ancestor==null ) {
...
//				System.err.println("Whoops. Tried to access invalid child");
	...
	public void align(int alignOrIndent, TerminalNode node) {
		int[] deltaChild = com.dh.antlr.sqlbuff.Trainer.triple(alignOrIndent);
		int deltaFromAncestor = deltaChild[0];
		int childIndex = deltaChild[1];
		ParserRuleContext earliestLeftAncestor = earliestAncestorStartingWithToken(node);
		ParserRuleContext ancestor = com.dh.antlr.sqlbuff.Trainer.getAncestor(earliestLeftAncestor, deltaFromAncestor);
...
//				System.err.println("Whoops. Tried to access invalid child");
	...
	public int[] getFeatures(com.dh.antlr.sqlbuff.InputDocument doc, int tokenIndexInStream) {
	...
	public int emitCommentsToTheLeft(int tokenIndexInStream, int injectNL_WS) {
		...
		if ( hiddenTokensToLeft!=null ) {
			...
			boolean hasComment = com.dh.antlr.sqlbuff.Trainer.hasCommentToken(hiddenTokensToLeft);
			if ( hasComment ) {
				...
				for (Token hidden : truncated) {
					...
					if ( hiddenText.matches("\\n+") ) {
						line += com.dh.antlr.sqlbuff.Tool.count(hiddenText, '\n');
						charPosInLine = 0;
					}
					else {
	                                              ...
	                                              boolean collectAnalysis)
	{
		...
		int actualWS = com.dh.antlr.sqlbuff.Trainer.getInjectWSCategory(originalTokens, tokenIndexInStream);
		...
		int actualAlignCategory = com.dh.antlr.sqlbuff.Trainer.getAlignmentCategory(originalDoc, nodeWithOriginalToken, indentSize);
		...
		a.actualWS = com.dh.antlr.sqlbuff.Trainer.getInjectWSCategory(originalTokens, tokenIndexInStream);
		a.actualAlign = actualAlignCategory;
		return a;
	}

	public static String getWSCategoryStr(int injectNL_WS) {
		int[] elements = com.dh.antlr.sqlbuff.Trainer.triple(injectNL_WS);
	...
	public static String getHPosCategoryStr(int alignOrIndent) {
		int[] elements = com.dh.antlr.sqlbuff.Trainer.triple(alignOrIndent);
